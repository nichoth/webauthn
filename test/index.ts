...
async function createMockWebAuthnSetup (dataToSign:string) {
    ...
    const challengeBuffer = await crypto.subtle.digest('SHA-256', dataBuffer)
    ...
    const challengeBase64url = btoa(String.fromCharCode(...new Uint8Array(challengeBuffer)))
    ...
    // Compute the data that WebAuthn actually signs: authData || SHA-256(clientDataJSON)
    const clientDataHash = await crypto.subtle.digest('SHA-256', clientDataJSONBuffer)
...
test('base64ToBuffer handles URL-safe base64', t => {
    ...
    t.deepEqual(Array.from(restored), Array.from(bytes), 'URL-safe base64 should round-trip')
})
...
test('sign returns a Signature object with all required fields', async t => {
    ...
    try {
        ...
        t.ok(signature.authenticatorData, 'signature should have authenticatorData')
...
test('verify returns false for a tampered signature', async t => {
    ...
    try {
        ...
        tampered[0] ^= 0xff
        const tamperedSig = { ...signature, signature: bufferToBase64(tampered.buffer) }
